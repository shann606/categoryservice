...
public interface SubCategoryRepository extends JpaRepository<SubCategory, UUID> {
	...
	@Query("""
			SELECT new com.exp.categoryservice.dto.CategoriesAndSubCategories(s.id,s.name)
			FROM SubCategory s
			WHERE s.catId=:categoryId
			ORDER BY s.info.createdOn ASC
			""")
	List<CategoriesAndSubCategories> getSubCategories(@Param("categoryId") UUID categoryId);
}
